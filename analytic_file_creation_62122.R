...
#To save the log of this file

#Copy the log of this file

# logfile <- file("./log_analytic_file_creation.txt")
...
# covars <- read.csv(file = "C:/Users/rsiwa/OneDrive/Michigan_course/research_rotation2/original_data/2_v54_006_hsd_06092021_20210913121711.csv")
...
#all metabolites in the metabolites file has confidence id = 2a, which means Highly confident match (AM-RT)	Accurate mass 
#and retention time based on standard
...
#Create parents phthalates: https://www.cdc.gov/exposurereport/pdf/fourthreport.pdf
# 
# phthalates3 <- phthalates3 %>% mutate(par_analyte = case_when(ANALYTE == "MEHP" ~ "DEHP",
#                                                               ANALYTE == "MECPP" ~ "DEHP",
#                                                               ANALYTE == "MEHHP" ~ "DEHP",
#                                                               ANALYTE == "MEOHP" ~ "DEHP",
#                                                 
...
#                                                               ANALYTE == "MEP" ~ "MEP",
#                                       
#                                                     #MONP classified as both DNP and DiNP
#                                                               ANALYTE == "MNP" ~ "MNP",
#                                                               ANALYTE == "MCOP" ~ "MCOP",
#                                                               ANALYTE == "MONP" ~ "MONP",
#                                                 
#                                                               ANALYTE == "MCOCH" ~ "DINCH",
#                                                               ANALYTE == "MHINCH" ~ "DINCH",
#                                                 
#                                                               ANALYTE == "MCPP" ~ "MCPP",
#                                                 
#                                                               ANALYTE == "MCNP" ~ "MCNP",
#                                                 
#                                                               ANALYTE == "MECPTP" ~ "DEHTP",
#                                                               ANALYTE == "MEHHTP" ~ "DEHTP"))
...
# phthalates3_wide <- phthalates3_wide %>% select(StudyID, VISITID, CONCUNITS, DEHP, DBP, DIBP, DINCH, DEHTP, MBZP, MEP, MCPP, MCNP, MNP, MCOP, MONP)
...
gm_list <- lapply(phthalates_selected[-1], function(col) {
...
#Some phthalates have a lot of missing values
#MHBP, MHIBP, MNP: 36%
#DINCH: 14%
#DEHTP, MONP: 14%
...
temp1 <- metabolites_t %>% anti_join(phthalates4_wide2, by = c("StudyID"))

#Visit 3 phthalates data not available for StudyID = 3034
...
#BMI calculation: https://www.cdc.gov/healthyweight/assessing/bmi/adult_bmi/index.html#Interpreted - manually calculating BMI did not reduce the number of 
#missing values for BMI.

# covars <- covars %>% select(StudyID, wtprepreg, isage, fvcurrht_foot, fvcurrht_inch) %>% mutate(bmi = wtprepreg/((fvcurrht_foot*12 + fvcurrht_inch)^2)*703)
...
###########################################################################################################################################
#To obtain a file with all metabolites, uncomment below 

# 
# phthalates4_wide_chk2 = phthalates4_wide_chk
# 
# 
# #Divide phthalates4_wide_chk2by VISITID, apply the above function, and then recombine them again
# 
# phthalates4_wide_v1 <- phthalates4_wide_chk2%>% filter(VISITID == 1)
# phthalates4_wide_v2 <- phthalates4_wide_chk2%>% filter(VISITID == 2)
# phthalates4_wide_v3 <- phthalates4_wide_chk2%>% filter(VISITID == 3)
# 
# #For visit = 1
# 
# for(i in 4:22) {
...
# 
# 
# names(phthalates4_wide2)

# saveRDS(phthalates4_wide2, "C:/Users/rsiwa/OneDrive/Michigan_course/research_rotation2/phthalates4_wide2_allphthalates.rds")
