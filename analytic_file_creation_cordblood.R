...
# sg <- read.csv(file = "-1_v54_006_specific_gravity_20200616100219.csv" )
...
# covars <- read.csv(file = "subject-vars.csv", check.names = FALSE) %>% 
#   mutate(PPSEX1 = PPSEX, ISAGE1 = ISAGE, BIRTHWEIGHT1 = BIRTHWEIGHT) %>% select(-c(PPSEX, ISAGE, BIRTHWEIGHT))
...
#link names in t_metabolites_neg/pos to metab_names
names(t_metabolites_neg)
names(t_metabolites_pos)
...
check1 <- metabolites_t %>% anti_join(metabolites, by = "Sample ID")
...
chk <- data.frame(ramfile = metabolites_t$`Glutamine-N`,
Gayafile = metabolites$`Glutamine-N`)
...
check3 <- phthalates_met %>% filter(`Study ID` == 2913) 
check4 <- covars2 %>% filter(`Study ID` == 2913) #This ID has two records 

names(phthalates_met)
