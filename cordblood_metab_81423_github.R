...
#revised on 3/17/2024 to switch colors for positive and negative in manhattan and heat plot
...
load(
  "C:/Users/rsiwa/OneDrive/Michigan_course/research_rotation2/cord_blood_data/phthalates_met2_cordblood.RData"
)
classes <- read.csv(
  "C:/Users/rsiwa/OneDrive/Michigan_course/research_rotation2/cord_blood_data/list_of_cordblood_metabolites2_csv.csv"
) %>% distinct()
classes <- classes %>% mutate(class = trimws(class), Class_new2 = class) %>% rename("Metabolites" = "name")

phthalates_all <- read.csv(
  "C:\\Users\\rsiwa\\Dropbox (University of Michigan)\\Ram_shared\\data\\2_v54_006_Phthalates_no_dup_03092021_20210625153132.csv"
) %>%
...
nrow(phthalates_met2)

# write.csv(phthalates_cordblood_wide, "phthalates_cordblood_wide.csv")
...
corph <- corrplot(
  corr = cor(phthalates, method = "spearman", use = "complete"),
  method = "number",
  type = "lower"
)
...
for (i in metstart:metend)
  for (j in phstart:phend) {
    fit_lin3[i - metstart + 1, j - phstart + 1] <- summary(
      lm(
        phthalates_met2[, i] ~ phthalates_met2[, j] + ISAGE + as.factor(PPSEX) +  prebmi,
        data = phthalates_met2
      )
    )$coefficients[17] #p-values
    
    fit_lin4[i - metstart + 1, j - phstart + 1] <- summary(
      lm(
        phthalates_met2[, i] ~ phthalates_met2[, j] + ISAGE + as.factor(PPSEX) +  prebmi,
        data = phthalates_met2
      )
    )$coefficients[2] #betas
    
    fit_lin5[i - metstart + 1, j - phstart + 1] <- summary(
      lm(
        phthalates_met2[, i] ~ phthalates_met2[, j] + ISAGE + as.factor(PPSEX) +  prebmi,
        data = phthalates_met2
      )
...
fit_lin3_adjust <- cbind.data.frame(Metabolite = fit_lin3[, 11], apply(fit_lin3[, 1:10], 2, function(x)
...
fit_lin34_adjust_long <- fit_lin34_adjust_long %>%
  ...
  mutate(
    Phthalate =  stringi::stri_replace_all_regex(
      Phthalates,
      pattern = c("log_", "_GM", "SG_"),
      replacement = c("", ""),
      vectorize_all = FALSE
    )
  ) %>% select(-Phthalates)
...
fit_lin34_adjust_long <- fit_lin34_adjust_long %>% mutate(
  `Beta coefficient` = beta_sign,
  Label = ifelse(adj_p <= pthres, Metabolite, ""),
  Metabolite = toupper(Metabolite),
  Phthalate = toupper(Phthalate)
)
...
# write.csv(fit_lin34_adjust_long, "manplot_cordblood_data.csv")
#############################################################################################################################
...
origpvalue_cord2 <- origpvalue_cord %>%
  ...
  pivot_longer(
    cols = log_SG_MBzP_GM:log_SG_DEHTP_GM,
    names_to = "Phthalates",
    values_to = "Unadj_p"
  ) %>%
  mutate(
    Metabolites = toupper(Metabolites),
    Phthalates = toupper(Phthalates),
    Phthalates = stringi::stri_replace_all_regex(
      Phthalates,
      pattern = c("LOG_", "_GM", "SG_"),
      replacement = c("", ""),
      vectorize_all = FALSE
...
blod_summary1 <- cord_orig_data2 %>% group_by(ANALYTE, VISITID) %>% dplyr::summarize(detrate = 100 -
                                                                                       sum(BLOD) / 81 * 100) %>% dplyr::filter(VISITID == 1)
blod_summary2 <- cord_orig_data2 %>% group_by(ANALYTE, VISITID) %>% dplyr::summarize(detrate = 100 -
                                                                                       sum(BLOD) / 79 * 100) %>% dplyr::filter(VISITID == 2)
blod_summary3 <- cord_orig_data2 %>% group_by(ANALYTE, VISITID) %>% dplyr::summarize(detrate = 100 -
                                                                                       sum(BLOD) / 71 * 100) %>% dplyr::filter(VISITID == 3)

blod_summary <- rbind.data.frame(blod_summary1, blod_summary2, blod_summary3)

blod_summary_chk <- cord_orig_data2 %>% group_by(ANALYTE) %>% dplyr::summarize(detrate = 100 -
                                                                                 sum(BLOD) / 231 * 100,
                                                                               nasum = sum(is.na(BLOD)) / 231 * 100)
...
cord_orig_data_wide <- cord_orig_data2 %>% select(StudyID, VISITID, ANALYTE, BLOD, CONC) %>% group_by(StudyID, ANALYTE) %>%
  ...
  pivot_wider(names_from = c("ANALYTE"),
              # The columns to use as column names
